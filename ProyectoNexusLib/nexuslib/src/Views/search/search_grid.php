<?php
use App\Models\Libro;

include __DIR__ . '/../layouts/header.php';

$resultados = $resultados ?? [];
$q = trim((string) ($_GET['q'] ?? ''));
$type = trim((string) ($_GET['type'] ?? 'title'));
$page = max(1, (int) ($_GET['page'] ?? 1));
$typeLabel = ($type === 'author') ? 'nombres de autores' : 'títulos de libros';
?>

<div class="container-fluid px-4">
    <div class="welcome-section text-center py-4">
        <h2 class="fw-bold text-white">Resultados para <span class="text-cyan">"<?= htmlspecialchars($q) ?>"</span> en <?= htmlspecialchars($typeLabel) ?></h2>
    </div>

    <div class="books-grid">
        <?php if (empty($resultados)): ?>
            <div class="alert alert-dark border-secondary text-secondary w-100">
                <i class="bi bi-info-circle me-2"></i>
                No hay resultados para mostrar
            </div>
        <?php else: ?>
            <?php foreach ($resultados as $item):
                $libro = $item['libro'] ?? null;
                if (!($libro instanceof Libro)) {
                    continue;
                }
                $linkId = !empty($libro->isbn) ? $libro->isbn : ($libro->id_libro ?? '');
            ?>
                <a href="index.php?action=details&id=<?= urlencode($linkId) ?>" class="text-decoration-none text-reset d-block book-card-link">
                    <div class="book-card">
                        <div class="book-cover">
                            <?php if (!empty($libro->portada_url)): ?>
                                <img src="<?= htmlspecialchars($libro->portada_url) ?>" alt="<?= htmlspecialchars($libro->titulo ?? '') ?>">
                            <?php else: ?>
                                <div class="no-cover d-flex align-items-center justify-content-center bg-dark h-100 w-100">
                                    <i class="bi bi-book text-secondary fs-1"></i>
                                </div>
                            <?php endif; ?>
                        </div>
                        <div class="book-card-body">
                            <span class="book-author d-block w-100 text-truncate" title="<?= htmlspecialchars($libro->autor) ?>"><?= htmlspecialchars($libro->autor) ?></span>
                            <span class="book-cat-label d-block w-100 text-truncate" title="<?= htmlspecialchars($libro->categoria ?? 'General') ?>"><?= htmlspecialchars($libro->categoria ?? 'General') ?></span>
                        </div>
                    </div>
                </a>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>

    <div class="container text-center my-4">
        <button type="button" id="load-more-search" class="load-more-btn" aria-label="Cargar más resultados">Cargar más resultados</button>
    </div>
</div>

<script>
(function () {
    const btn = document.getElementById('load-more-search');
    const grid = document.querySelector('.books-grid');
    if (!btn || !grid) return;

    const query = <?= json_encode($q) ?>;
    const type = <?= json_encode($type) ?>;
    let page = <?= $page ?>;

    btn.addEventListener('click', async function () {
        const originalText = btn.textContent;
        btn.disabled = true;
        btn.textContent = 'Cargando...';

        const targetPage = page + 1;
        const url = 'index.php?action=search-more&q=' + encodeURIComponent(query) + '&type=' + encodeURIComponent(type) + '&page=' + targetPage + '&ajax=1';

        try {
            const resp = await fetch(url, { credentials: 'same-origin' });
            if (!resp.ok) throw new Error('Network response was not ok');
            const data = await resp.text();

            if (data.trim() === '') {
                btn.textContent = 'No hay más resultados';
                btn.disabled = true;
            } else {
                grid.insertAdjacentHTML('beforeend', data);
                page = targetPage;
                btn.textContent = originalText;
                btn.disabled = false;
            }
        } catch (err) {
            console.error(err);
            btn.textContent = originalText;
            btn.disabled = false;
        }
    });
})();
</script>

<?php include __DIR__ . '/../layouts/footer.php'; ?>

